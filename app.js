...
function renderNotes(){
 ...
 const preview=n=>esc(stripHtml(n.body).slice(0,160))||'<span class="note-no-preview">No content yet</span>';
 let body;
 ...
 else if(view==='grid')body=`<div class="task-grid note-grid notes-view">${list.map(n=>`<div class="task-card note-card ${n.pinned?'pinned':''}" ${open(n)}><div class="note-card-top"><div class="task-title">${docIcon}${esc(n.title||'Untitled')}</div>${pinBtn(n)}</div><div class="note-card-preview">${preview(n)}</div><div class="note-card-foot">${meta(n)}</div></div>`).join('')}</div>`;
 else body=`<div class="note-list notes-view">${list.map(n=>`<div class="note-row ${n.pinned?'pinned':''}" ${open(n)}><span class="note-row-icon">${docIcon}</span><div class="note-row-main"><div class="note-row-title">${esc(n.title||'Untitled')}</div><div class="note-row-preview">${preview(n)}</div><div class="note-row-meta">${meta(n)}</div></div>${pinBtn(n)}</div>`).join('')}</div>`;
 const viewBtn=(v,icon,label)=>`<button type="button" class="${view===v?'active':''}" data-notes-view="${v}" aria-pressed="${view===v}" title="${label} view" aria-label="${label} view"><span aria-hidden="true">${icon}</span><b>${label}</b></button>`;
 root.innerHTML=`<div class="hero"><div><div class="eyebrow">Quick notes</div><h2>One place for the things you don't want to forget.</h2><p>Use subject notebooks for lessons. Use general notes here for reminders, ideas, checklists and anything that doesn't belong to one class.</p></div><button class="primary" data-new-general>+ New note</button></div><div class="card section-gap"><div class="card-head notes-head"><h2>General notes</h2><div class="notes-controls"><input class="input" id="notesSearch" value="${esc(q)}" placeholder="Search notes…" aria-label="Search notes"><select class="select" id="notesSort" aria-label="Sort notes"><option value="newest" ${sort==='newest'?'selected':''}>Newest first</option><option value="oldest" ${sort==='oldest'?'selected':''}>Oldest first</option><option value="az" ${sort==='az'?'selected':''}>A–Z</option><option value="za" ${sort==='za'?'selected':''}>Z–A</option></select><div class="seg notes-view-seg" role="group" aria-label="Notes view">${viewBtn('list','☷','List')}${viewBtn('grid','▦','Grid')}</div><span class="pill">${list.length} note${list.length===1?'':'s'}</span></div></div><div class="card-body">${body}</div></div>`;
}
...
function wire(){ $$('.nav-item[data-view]').forEach(b=>b.onclick=()=>setView(b.dataset.view));$('#sideCollapse').onclick=toggleSidebar;restoreSidebar();sidebarTooltips();$('#mobileMenu').onclick=()=>{$('#sidebar').classList.add('mobile-open');$('#scrim').classList.add('show')};$('#scrim').onclick=closeMobile;$('#quickAdd').onclick=()=>quickAdd();$('#globalSearchBtn').onclick=()=>quickSearch();$('#themeToggle').onclick=()=>{state.settings.theme=document.body.classList.contains('dark')?'light':'dark';document.body.classList.toggle('dark',state.settings.theme==='dark');save()};document.addEventListener('keydown',e=>{if((e.key==='Enter'||e.key===' ')&&e.target.matches?.('[data-general][role="button"]')){e.preventDefault();e.target.click();return}if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='k'){e.preventDefault();quickSearch()}else if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='s'){const b=$('#saveGeneral')||$('#view-subjects.active [data-save-lesson]');if(b){e.preventDefault();b.click()}}});}
