...
// Light up H2 / H3 / Normal for the line the caret is on, so it is clear which style you are typing in.
document.addEventListener('selectionchange',()=>{
 const s=getSelection(),n=s.rangeCount?s.getRangeAt(0).startContainer:null;
 const editor=(n?.nodeType===1?n:n?.parentElement)?.closest('.note-editor');if(!editor)return;
 const toolbar=document.querySelector(`.editor-toolbar[data-target="${editor.id}"]`);if(!toolbar)return;
 const v=String(document.queryCommandValue('formatBlock')||'').toLowerCase();
 const block=v==='h2'||v==='h3'?v:(v===''||v==='div'||v==='p'?'normal':'');
 toolbar.querySelectorAll('[data-block]').forEach(b=>{const on=b.dataset.block===block;b.classList.toggle('is-active',on);b.setAttribute('aria-pressed',String(on))});
});
...
function renderSubjects(){
 ...
 const visible=all.filter(x=>!q||(name(x)+' '+(x.s.prof||'')).toLowerCase().includes(q));
 ...
 else if(view==='list')body=`<div class="note-list subject-list notes-view">${list.map(({s,i,info})=>`<div class="note-row subject-row" data-subject="${s.id}" role="button" tabindex="0" aria-label="Open ${esc(name({s}))} notebook" style="--subject-color:${SUBJECT_COLORS[i%8]}"><span class="subject-row-badge" aria-hidden="true">${i+1}</span><div class="note-row-main"><div class="note-row-title">${esc(name({s}))}</div>${profLineHtml(s)}<div class="note-row-meta"><small>${info.text}</small></div></div><span class="pill">${info.openTasks} open task${info.openTasks===1?'':'s'}</span><span class="subject-row-go" aria-hidden="true">›</span></div>`).join('')}</div>`;
