...
function renderNotes(){const root=$('#view-notes');const sort=root.dataset.sort||'newest';const list=[...(state.notes.general||[])].sort((a,b)=>(sort==='oldest'?1:-1)*((a.updated||0)-(b.updated||0)));root.innerHTML=`<div class="hero"><div><div class="eyebrow">Quick notes</div><h2>One place for the things you don't want to forget.</h2><p>Use subject notebooks for lessons. Use general notes here for reminders, ideas, checklists and anything that doesn't belong to one class.</p></div><button class="primary" data-new-general>+ New note</button></div><div class="card section-gap"><div class="card-head"><h2>General notes</h2><div style="display:flex;align-items:center;gap:10px"><select class="select" id="notesSort"><option value="newest" ${sort==='newest'?'selected':''}>Newest first</option><option value="oldest" ${sort==='oldest'?'selected':''}>Oldest first</option></select><span class="pill">${list.length} notes</span></div></div><div class="card-body"><div class="task-grid">${list.map(n=>`<div class="task-card" data-general="${n.id}"><div class="task-title">${esc(n.title||'Untitled')}</div><div style="font-size:11px;color:var(--muted);margin-top:8px">${esc((n.body||'').replace(/\s+/g,' ').slice(0,130))}</div></div>`).join('')||'<div class="empty" style="grid-column:1/-1">No general notes yet.</div>'}</div></div></div>`}
